...
pub enum DownloadEvent {
    Progress { percentage: f32, speed: String, eta: String },
}
...
pub async fn download_media(
    ...
    let sidecar = app.shell().sidecar("yt-dlp").map_err(|e| {
        AutoSubError::Download(format!("Failed to find yt-dlp sidecar: {}", e))
    })?;

    // Use app cache dir if no output dir provided
    let output_dir = opts.output_dir.unwrap_or_else(|| {
        app.path().app_cache_dir().expect("Failed to get cache dir").join("downloads")
    });
    ...
    // Improved Regexes
    let re_progress = Regex::new(r"\[download\]\s+([\d.]+)\%\s+of\s+[\d.]+\w+\s+at\s+([\d.]+\w+/s)\s+ETA\s+([\d:]+)").unwrap();
    let re_dest = Regex::new(r"\[(?:destination|ffmpeg|ExtractAudio)\]\s+(?:Destination:\s+)?(.+)").unwrap();
    ...
    while let Some(event) = rx.recv().await {
        if let tauri_plugin_shell::process::CommandEvent::Stdout(line) = event {
            ...
            if let Some(caps) = re_progress.captures(&line_str) {
                let percentage = caps.get(1).and_then(|m| m.as_str().parse::<f32>().ok()).unwrap_or(0.0);
                ...
                app.emit("download-progress", DownloadEvent::Progress { percentage, speed, eta }).ok();
            } else if let Some(caps) = re_dest.captures(&line_str) {
                let path_str = caps.get(1).map(|m| m.as_str().trim().trim_matches('"')).unwrap_or("");
                if !path_str.is_empty() && !path_str.contains("Merging formats") {
                    let path = PathBuf::from(path_str);
                    final_path = Some(output_dir.join(&path));
                    title = path.file_name().map(|n| n.to_string_lossy().to_string()).unwrap_or_else(|| path_str.to_string());
                }
            ...
            if payload.code != Some(0) {
                return Err(AutoSubError::Download(format!("yt-dlp exited with error code {:?}", payload.code)));
            }
        }
    }
    ...
    if let Some(path) = final_path {
        ...
        if opts.save_local {
            ...
            let dest_name = path.file_name().ok_or_else(|| AutoSubError::Download("Invalid filename".into()))?;
        ...
        Err(AutoSubError::Download("Could not determine downloaded file path".into()))
    }
}
